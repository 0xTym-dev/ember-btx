...
class StratumClient:
    ...
    def _on_notify(self, params: list[Any]) -> None:
        ...
        # Diagnose: wie leicht ist die Share gerade relativ zum Block-Target?
        try:
            exp = int(job.bits[:2], 16)
            block_target = int(job.bits[2:], 16) << (8 * (exp - 3))
            ratio = int(job.target, 16) / block_target
            log.info("share_target=%s... ratio_vs_block=%.1fx easier",
                     job.target[:16], ratio)
    ...
    async def _solver_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                # Stream-submit: a patched solver daemon emits each found
                # share mid-slice; submitting it now (instead of at slice
                # end) beats tip moves that would turn it stale.
                # _submit_share dedupes vs the slice-end "shares" array.
                self.solver.on_stream_share = self._make_stream_share_cb(job)
    ...
    def _make_stream_share_cb(self, job: Job):
        """Build the per-slice callback that submits a streamed share now.

        Invoked synchronously from the wrapper's async stdout read loop, so
        scheduling a task here is safe. Stale guard mirrors the slice-end
        path: if the tip moved to a different parent, the pool would reject
        the share anyway.
        """
        def _cb(rec: dict) -> None:
            try:
                nonce = int(rec.get("nonce64"))
            except (TypeError, ValueError):
                return
            cur = self._current_job
            if cur is not None and cur.previousblockhash != job.previousblockhash:
                return
            res = SolveResult(
                found=True, tries_used=0, elapsed_s=0.0,
                nonce=nonce, digest_hex=rec.get("matmul_digest"),
                ntime=None, raw_output=rec,
                is_block=bool(rec.get("is_block", False)),
            )
            _st = getattr(self, "_submit_tasks", None)
            if _st is None:
                _st = set()
                self._submit_tasks = _st
            _t = asyncio.create_task(self._submit_share(job, res))
            _st.add(_t)
            _t.add_done_callback(_st.discard)
        return _cb
    ...
    async def _submit_share(self, job: Job, share: SolveResult) -> None:
        ...
        if share.nonce is None:
            ...
            return
        # Dedupe: with stream-submission the same share arrives again in the
        # slice-end "shares" array — submit each (parent, nonce) only once.
        key = (job.previousblockhash, share.nonce)
        seen = getattr(self, "_submitted_share_keys", None)
        if seen is None:
            seen = set()
            self._submitted_share_keys = seen
        if key in seen:
            return
        if len(seen) > 8192:
            seen.clear()
