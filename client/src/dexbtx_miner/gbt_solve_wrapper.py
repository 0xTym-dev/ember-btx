...
class GbtSolveWrapper:
    ...
    async def _ensure_daemon(self, challenge: SolveChallenge) -> asyncio.subprocess.Process:
        ...
        # Continuously drain the solver's stderr AFTER the ready handshake.
        # Two reasons: (1) the solver now emits ongoing structured events on
        # stderr (dev-fee disclosure + per-round records); an undrained pipe
        # fills its ~64KB buffer and blocks the solver mid-mine. (2) Surfacing
        # those lines in our log makes the dev fee transparent to the operator.
        self._stderr_task = asyncio.create_task(self._drain_stderr(self._daemon))
        self._matmul_params = shape
        return self._daemon

    async def _drain_stderr(self, daemon) -> None:
        """Forward solver stderr lines to our log, one per event, until EOF.

        Best-effort: never raises into the caller. Cancelled on daemon
        shutdown/restart. Keeps the stderr pipe empty so std::cerr in the
        solver never blocks.
        """
        ...
        try:
            while True:
                ...
                if text:
                    log.info("solver: %s", text)
        except asyncio.CancelledError:
            raise
        except Exception as e:  # noqa: BLE001 — telemetry drain must not crash mining
            log.debug("stderr drain ended: %s", e)
    ...
    async def _send_job_with_retry(
        ...
        for attempt in (1, 2):
            try:
                ...
                # Read JSON lines back until the final slice record.
                # A patched daemon (July 2026+) streams each found share
                # immediately as {"share_stream": true, ...} so the client
                # can submit it before the tip moves; the final record still
                # carries the full "shares" array (client dedupes by nonce).
                while True:
                    ...
                    if rec.get("share_stream"):
                        cb = getattr(self, "on_stream_share", None)
                        if cb is not None:
                            try:
                                cb(rec)
                            except Exception as e:  # never kill the read loop
                                log.debug("on_stream_share callback failed: %s", e)
                        continue
                    if "error" in rec:
